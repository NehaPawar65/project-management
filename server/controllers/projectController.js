...
export const createProject = async (req, res) => {
  try {
    const {useId} = await req.auth();
    const {workspaceId, description, name, status, status_date, end_date,team_members, team_lead, progress, priority} = req.body;
    ...
    if(!workspace.members.some(member => member.userId === useId && member.role === 'admin')) {
    ...
    const project = await prisma.project.create({
      data: {
        ...
        end_date: end_date? new(end_date): null,
      }
    });
    ...
    const projectWithMembers = await prisma.project.findUnique({
      where: { id: project.id },
      include: { members: { include: { user: true } },
      tasks:{ include: { assigned_to: true, comments: {include: { user: true } } } },
      owner: true,
    }
    });
...
export const updateProject = async (req, res) => {
  try {
    ...
    if(!workspace.members.some(member => member.userId === useId && member.role === 'admin')) {
        ...
        }else if(project.team_lead != useId){
...
export const addMemberToProject = async (req, res) => {
  try {
    ...
    const existingMember = project.members.find((member)=>member.emailId === email )
